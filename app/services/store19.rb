...
class Store19 
    def products
        products = open(ENV['lcbo_store19_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "St Clair & Keele")
            
            if store == nil 
                store = Store.new(name: "St Clair & Keele")
                store.address_line_1 = "2151 St. Clair Ave. W."
                store.address_line_2 = nil
                store.city = "Toronto"
                store.zipcode = "M5N1K5"
                store.phone_number = "(416) 653-8202"
