...
class Store10 
    def products
        products = open(ENV['lcbo_store10_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Yonge & Summerhill")

            if store == nil 
                store = Store.new(name: "Yonge & Summerhill")
                store.address_line_1 = "10 Scrivener Square"
                store.address_line_2 = "Summerhill Subway"
                store.city = "Toronto"
                store.zipcode = "M4W3Y9"
                store.phone_number = "(416) 922-0403"
