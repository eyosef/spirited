...
class Store16 
    def products
        products = open(ENV['lcbo_store16_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Yonge & Empress") 
            
            if store == nil 
                store = Store.new(name: "Yonge & Empress")
                store.address_line_1 = "5095 Yonge Street A4"
                store.address_line_2 = nil
                store.city = "North York"
                store.zipcode = "M2N6Z4"
                store.phone_number = "(416) 226-3949"
