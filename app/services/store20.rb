...
class Store20 
    def products
        products = open(ENV['lcbo_store20_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Lake Shore & Islington")

            if store == nil 
                store = Store.new(name: "Lake Shore & Islington")
                store.address_line_1 = "2762 Lake Shore Boulevard West"
                store.address_line_2 = nil
                store.city = "Etobicoke"
                store.zipcode = "M8V1H1"
                store.phone_number = "(416) 251-2021"
