...
class Store6 
    def products
        products = open(ENV['lcbo_store6_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Queen & Lansdowne")

            if store == nil 
                store = Store.new(name: "Queen & Lansdowne")
                store.address_line_1 = "1357 Queen St. W."
                store.address_line_2 = nil 
                store.city = "Toronto"
                store.zipcode = "M6K1M1"
                store.phone_number = "(416) 532-7283"
