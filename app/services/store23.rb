...
class Store23 
    def products
        products = open(ENV['lcbo_store23_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Barton & Kenilworth")
            
            if store == nil 
                store = Store.new(name: "Barton & Kenilworth")
                store.address_line_1 = "1149 Barton Street East"
                store.address_line_2 = "Centre Mall"
                store.city = "Hamilton"
                store.zipcode = "L8H2V2"
                store.phone_number = "(905) 544-8010"
