...
class Store22 
    def products
        products = open(ENV['lcbo_store22_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Big Bay Point Rd & Yonge")

            if store == nil 
                store = Store.new(name: "Big Bay Point Rd & Yonge")
                store.address_line_1 = "461 Big Bay Point Road"
                store.address_line_2 = "Meadowlands Centre"
                store.city = "Barrie"
                store.zipcode = "L4N3Z3"
                store.phone_number = "(705) 739-0162"
