...
class Store9 
    def products
        products = open(ENV['lcbo_store9_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Bloor & Ossington")

            if store == nil 
                store = Store.new(name: "Bloor & Ossington")
                store.address_line_1 = "879 Bloor Street West"
                store.address_line_2 = nil
                store.city = "Toronto"
                store.zipcode = "M6G1M4"
                store.phone_number = "(416) 536-4340"
