...
class Store17 
    def products
        products = open(ENV['lcbo_store17_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Front & Sherbourne")
            
            if store == nil 
                store = Store.new(name: "Front & Sherbourne")
                store.address_line_1 = "222 Front Street East"
                store.address_line_2 = nil
                store.city = "Toronto"
                store.zipcode = "M5A1E7"
                store.phone_number = "(416) 214-0311"
