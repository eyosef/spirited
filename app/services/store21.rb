...
class Store21 
    def products
        products = open(ENV['lcbo_store21_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Golf Links & Hwy 403")
            
            if store == nil 
                store = Store.new(name: "Golf Links & Hwy 403")
                store.address_line_1 = "737 Gold Links Road"
                store.address_line_2 = "Meadowlands Centre"
                store.city = "Ancaster"
                store.zipcode = "L9K1L5"
                store.phone_number = "(905) 304-9608"
