...
class Store8 
    def products
        products = open(ENV['lcbo_store8_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "St Clair & Oakwood")

            if store == nil 
                store = Store.new(name: "St Clair & Oakwood")
                store.address_line_1 = "908 St. Clair Avenue West"
                store.address_line_2 = nil
                store.city = "Toronto"
                store.zipcode = "M6C1C6"
                store.phone_number = "(416) 654-4240"
