...
class Store14 
    def products
        products = open(ENV['lcbo_store14_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Spadina & Dundas")

            if store == nil 
                store = Store.new(name: "Spadina & Dundas")
                store.address_line_1 = "337 Spadina Avenue"
                store.address_line_2 = nil
                store.city = "Toronto"
                store.zipcode = "M5T2E9"
                store.phone_number = "(416) 597-0145"
