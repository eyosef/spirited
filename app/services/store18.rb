...
class Store18 
    def products
        products = open(ENV['lcbo_store18_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "King & Dufferin")
            
            if store == nil 
                store = Store.new(name: "King & Dufferin")
                store.address_line_1 = "85 Hanna Avenue, Unit 103"
                store.address_line_2 = nil
                store.city = "Toronto"
                store.zipcode = "M6K3S3"
                store.phone_number = "(416) 538-9200"
