...
class Store12 
    def products
        products = open(ENV['lcbo_store12_key'])
        ...
        products_data["result"].each do |product|
            ...
            store = Store.find_by(name: "Parliament and Wellesly")

            if store == nil 
                store = Store.new(name: "Parliament and Wellesly")
                store.address_line_1 = "512 Parliament Street a"
                store.address_line_2 = nil
                store.city = "Toronto"
                store.zipcode = "M4X1P4"
                store.phone_number = "(416) 923-6966"
